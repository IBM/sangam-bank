...
export class AmountTransferComponent {
  ...
  getBalance() {
    this.homeService
      .getbalance(this.amountTransferForm.value.fromAccId)
      .subscribe(
        (response: any) => {
        ...
        (error: any) => {
          this.alertService.error('Error getting user current balance', {
            autoClose: true,
          });
        }
      );
  ...
  validateAmount() {
    if (this.amountTransferForm.value.transactionAmount > this.currentBalance) {
      this.amountTransferForm.controls['transactionAmount'].setErrors({
        incorrect: true,
      });
      this.transactionAmountError =
        'Transfer amount is greater than current balance';
    }
  }
  ...
  transferAmount() {
    ...
    if (transferDetails.transaction_amount > this.currentBalance) {
      ...
      this.homeService.transferAmount(transferDetails).subscribe(
        (response: any) => {
          this.isLoading = false;
          this.alertService.success('Transaction Successfull', {
            autoClose: true,
          });
        },
        () => {
          this.alertService.error(
            'Error in transferring amount to beneficiary account',
            { autoClose: true }
          );
          this.isLoading = false;
        }
