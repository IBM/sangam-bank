...
export class AgentDashboardComponent implements OnInit, AfterViewChecked {
  ...
  getCurrentTime() {
    ...
    const currentTime =
      this.datePipe.transform(currentDate, 'shortTime') || 'N/A';
    return currentTime;
  }
  ...
  isFraudulent(section: any): boolean {
    ...
    if (section.type === 'table' && Array.isArray(section.data)) {
      ...
      return section.data.some(
        (item: TableItem) => item.fraud_score !== undefined
      );
    }
    return false;
  }
    ...
    this.aiAisstantService.getAgentResponse(form.value.prompt).subscribe(
      ...
      (error: any) => {
        ...
        this.addResponseItem(
          'agent',
          'text',
          'Oops! We hit a snag. Could you try running your prompt again? Sometimes all it takes is a little nudge. If it still doesn’t work, I’m here to dive deeper and help out!'
        );
  ...
  sendChatResponse() {
    ...
    setTimeout(() => {
      ...
      let chatResponse =
        this.responseProcessingService.formatResponse(newResponseItem);
      chatResponse.sections.forEach((section, i = 0) => {
        if (section.type == 'table') {
          if (Array.isArray(section.data)) {
            section.data = this.flattenArray(section.data as any);
